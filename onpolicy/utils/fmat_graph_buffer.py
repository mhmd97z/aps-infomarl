...
from onpolicy.algorithms.fmat.utils.util import get_shape_from_obs_space, get_shape_from_act_space
...
class FmatReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space, env_name):
        ...
        self.action_log_probs = np.zeros(
            (self.episode_length, self.n_rollout_threads, num_agents, act_shape), dtype=np.float32)
        # self.action_log_probs = np.zeros(
        #     (self.episode_length, self.n_rollout_threads, num_agents, act_space.n), dtype=np.float32)
    ...
    def compute_returns(self, next_value, value_normalizer=None):
        ...
        for step in reversed(range(self.rewards.shape[0])):
            if self._use_popart or self._use_valuenorm:
                ...
                # print("reward", self.rewards[step].mean())
                # print("value_preds", self.value_preds[step].mean())
                # print("value norm", value_normalizer.denormalize(self.value_preds[step]))
