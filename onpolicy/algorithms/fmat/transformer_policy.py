import torch
import numpy as np
from onpolicy.algorithms.fmat.utils.util import get_shape_from_obs_space, update_linear_schedule
from onpolicy.algorithms.fmat.utils.util import check
...
class TransformerPolicy:
    ...
    def __init__(self, args, obs_space, cent_obs_space, act_space, num_agents, device=torch.device("cpu")):
        ...
        if args.env_name == "aps":
            if self.algorithm_name in ["fmat"]:
                from onpolicy.algorithms.fmat.ma_transformer_aps import MultiAgentTransformer as MAT
            else:
                raise NotImplementedError
            self.transformer = MAT(self.share_obs_dim, self.obs_dim, self.act_dim, num_agents,
                n_ap=args.env_args.simulation_scenario.number_of_aps, 
                n_ue=args.env_args.simulation_scenario.number_of_ues, 
                n_embd=args.n_embd, n_head=args.n_head, n_block=args.n_block,
            ...
            raise NotImplementedError("Environment {} is not supported".format(args.env_name))
    ...
    def get_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, masks, available_actions=None,
        ...
        # actions: (batch, n_agent, 1)
        # action_log_probs: (batch, n_agent, 1)
        # values: (batch, n_agent, 1)
        ...
        actions = actions.contiguous().view(-1, self.act_num)
        action_log_probs = action_log_probs.contiguous().view(-1, self.act_num)
    ...
    def evaluate_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, actions, masks,
        ...
        # entropy: [batch_size, n_agents, 1]
        action_log_probs, values, entropy = self.transformer(cent_obs, obs, actions, available_actions)

        action_log_probs = action_log_probs.contiguous().view(-1, self.act_num)
        values = values.contiguous().view(-1, 1)

        entropy = entropy.contiguous().view(-1, self.act_num)

        
        # action_log_probs = action_log_probs.reshape(-1, self.act_num)
        # values = values.reshape(-1, 1)
        # entropy = entropy.reshape(-1, self.act_num)
        # all_action_log = all_action_log.reshape(-1,self.act_dim)
