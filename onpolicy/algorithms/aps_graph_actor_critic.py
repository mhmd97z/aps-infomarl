...
class Aps_GR_Actor(nn.Module):
    ...
    def __init__(
        ...
        self.base = MLPBase(args, obs_shape=gnn_out_dim, override_obs_dim=None).to(torch.float32)
...
class Aps_GR_Critic(nn.Module):
    ...
    def __init__(
        ...
        if self._use_popart:
            self.v_out = init_(PopArt(self.hidden_size, 1, device=device).to(torch.float32))
        else:
            self.v_out = init_(nn.Linear(self.hidden_size, 1).to(torch.float32))
