...
class Aps(gym.Env):
    ...
    def compute_state_reward(self):
        ...
        x_mean = torch.tensor([-2.1114e+01, 5.6631e-03]).to(obs.dtype).to(obs.device)
        x_std = torch.tensor([2.1966, 1.8144]).to(obs.dtype).to(obs.device)

        obs = (obs - x_mean) / x_std
        # state = (state - x_mean) / x_std
